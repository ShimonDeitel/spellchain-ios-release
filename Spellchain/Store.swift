...
/// StoreKit 2 — one-time non-consumable unlock (`spellchain_pro_unlock`).
...
final class Store: ObservableObject {
    static let productID = "spellchain_pro_unlock"
    ...
    private var debugForcePro: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["SPELLCHAIN_FORCE_PRO"] == "1"
        #else
        return false
        #endif
    }
    ...
    init() {
        ...
        Task {
            #if DEBUG
            // SPELLCHAIN_NO_SK skips the product fetch so headless screenshot runs (no StoreKit
            // config / no App Store account) don't trigger the "Sign in to Apple Account" prompt.
            if ProcessInfo.processInfo.environment["SPELLCHAIN_NO_SK"] != "1" { await loadProduct() }
            #else
            await loadProduct()
            #endif
